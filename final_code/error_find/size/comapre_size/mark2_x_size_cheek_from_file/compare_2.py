...
file=open("x_max_min_thin_wall.txt","r")
...
for aaa in range(2,30):
    ...
    for data in file_data:
        max_x_mm, min_x_mm, z_pos_mm=data.split(" ")
    ...
    for give_hight in matching_x_vaules:
        print("higth",give_hight)
        print("x size ",matching_x_vaules[give_hight])
        ...
        for x_wiggle in range(-5,5):
            ...
            cv2.line(back_up, (x_max_postion_to_look, point_y), (x_min_postion_to_look, point_y), (255, 0, 0), 2)

            if comapre_vaule_x_max >z_dixstance_at_center_mm-5 and comapre_vaule_x_max <=z_dixstance_at_center_mm+5:
                x_max_wiggle_room=True

            if comapre_vaule_x_min > z_dixstance_at_center_mm - 5 and comapre_vaule_x_min <= z_dixstance_at_center_mm + 5:
